...
class Model(object):
    def __init__(self, n_uid, n_mid, n_cate, n_carte, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=False,
            ...
            if self.use_cartes:
                ...
                with tf.device(device):
                    for i, num in enumerate(n_carte):
                        print("carte num:", num)
                        self.carte_embedding_vars.append(
                            tf.get_variable("carte_embedding_var_{}".format(i), [num, EMBEDDING_DIM], trainable=True))
                        self.carte_batch_embedded.append(
                            tf.nn.embedding_lookup(self.carte_embedding_vars[i], self.carte_batch_ph[:, i, :]))
    ...
    def auxiliary_net(self, in_, stag='auxiliary_net'):
        ...
        dnn3 = tf.layers.dense(dnn2, 2 if self.use_softmax else 1, activation=None, name='f3' + stag,
                               reuse=tf.AUTO_REUSE)
        if self.use_softmax:
...
class Model_NCF(Model):
    def __init__(self, n_uid, n_mid, n_cate, n_carte, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=False,
        ...
        with tf.name_scope('ncf_embedding'):
            self.ncf_item_embedding_var = tf.get_variable("ncf_item_embedding_var", [n_mid, EMBEDDING_DIM],
                                                          trainable=True)
            self.ncf_cate_embedding_var = tf.get_variable("ncf_cate_embedding_var", [n_cate, EMBEDDING_DIM],
                                                          trainable=True)
...
def ProductLayer(feas, DIM, share=True):
    ...
    opnn = tf.reduce_sum((tf.multiply((tf.transpose(tf.reduce_sum(tf.multiply(p, w), axis=-1), [0, 2, 1])), q)),
                         axis=-1)
...
class Model_PNN(Model):
    def __init__(self, n_uid, n_mid, n_cate, n_carte, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=False,
                 use_softmax=True):
        super(Model_PNN, self).__init__(n_uid, n_mid, n_cate, n_carte, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE,
                                        use_negsampling, use_softmax=use_softmax)

        fea_list = [self.mid_batch_embedded, self.cate_batch_embedded,
                    tf.reduce_mean(self.mid_his_batch_embedded, axis=1),
                    tf.reduce_mean(self.cate_his_batch_embedded, axis=1)]
...
class Model_FM(Model):
    def __init__(self, n_uid, n_mid, n_cate, n_carte, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=False,
                 use_softmax=False):
        super(Model_FM, self).__init__(n_uid, n_mid, n_cate, n_carte, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE,
                                       use_negsampling, use_softmax=use_softmax)
        ...
        fea_list = [self.mid_batch_embedded, self.cate_batch_embedded,
                    tf.reduce_sum(self.mid_his_batch_embedded, axis=1),
                    tf.reduce_sum(self.cate_his_batch_embedded, axis=1)]
...
class Model_FFM(Model):
    def __init__(self, n_uid, n_mid, n_cate, n_carte, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=False,
                 use_softmax=False):
        super(Model_FFM, self).__init__(n_uid, n_mid, n_cate, n_carte, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE,
                                        use_negsampling, use_softmax=use_softmax)
        ...
        with tf.name_scope('FFM_embedding'):
            ...
            FFM_item_embedding_var = tf.get_variable("FFM_item_embedding_var", [n_mid, 3, EMBEDDING_DIM],
                                                     trainable=True)
            FFM_cate_embedding_var = tf.get_variable("FFM_cate_embedding_var", [n_cate, 3, EMBEDDING_DIM],
                                                     trainable=True)
...
class Model_DeepFFM(Model):
    def __init__(self, n_uid, n_mid, n_cate, n_carte, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=False,
                 use_softmax=False):
        super(Model_DeepFFM, self).__init__(n_uid, n_mid, n_cate, n_carte, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE,
                                            use_negsampling, use_softmax=use_softmax)
...
class Model_DeepFM(Model):
    def __init__(self, n_uid, n_mid, n_cate, n_carte, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=False,
                 use_softmax=False):
        super(Model_DeepFM, self).__init__(n_uid, n_mid, n_cate, n_carte, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE,
                                           use_negsampling, use_softmax=use_softmax)
        w_item_var = tf.get_variable("w_item_var", [n_mid, 1], trainable=True)
...
class Model_xDeepFM(Model):
    def __init__(self, n_uid, n_mid, n_cate, n_carte, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=False,
                 use_softmax=False):
        super(Model_xDeepFM, self).__init__(n_uid, n_mid, n_cate, n_carte, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE,
                                            use_negsampling, use_softmax=use_softmax)
...
class Model_PIN(Model):
    def __init__(self, n_uid, n_mid, n_cate, n_carte, EMBEDDING_DIM, HIDDEN_SIZE, BATCH_SIZE, SEQ_LEN=256):
        ...
        feas = [self.mid_batch_embedded, self.cate_batch_embedded,
                tf.reduce_sum(self.mid_his_batch_embedded * tf.reshape(self.mask, (BATCH_SIZE, SEQ_LEN, 1)), axis=1),
                tf.reduce_sum(self.cate_his_batch_embedded * tf.reshape(self.mask, (BATCH_SIZE, SEQ_LEN, 1)), axis=1)]
...
class Model_ONN(Model):
    def __init__(self, n_uid, n_mid, n_cate, n_carte, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=False,
                 use_softmax=False):
        super(Model_ONN, self).__init__(n_uid, n_mid, n_cate, n_carte, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE,
                                        use_negsampling, use_softmax=use_softmax)
        ...
        inp = tf.concat([self.uid_batch_embedded, self.mid_batch_embedded, self.cate_batch_embedded,
                         tf.reduce_mean(self.mid_his_batch_embedded, axis=1),
                         tf.reduce_mean(self.cate_his_batch_embedded, axis=1), onn], 1)
        logit = self.build_fcn_net(inp, use_dice=False)
        self.build_loss(logit)
...
class Model_DNN(Model):
    def __init__(self, n_uid, n_mid, n_cate, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=False,
        ...
        super(Model_DNN, self).__init__(n_uid, n_mid, n_cate, EMBEDDING_DIM, HIDDEN_SIZE,
                                        ATTENTION_SIZE,
                                        use_negsampling, use_softmax=use_softmax, use_coaction=use_coaction,
                                        use_cartes=use_cartes)
...
class Model_DIEN(Model):
    def __init__(self, n_uid, n_mid, n_cate, n_carte, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=True,
                 use_coaction=False):
